...
class ChatterbotData:
    ...
    def __init__(self, dirName):
        ...
        """
        print('creating Chatterbot obj')

        if os.path.isfile(os.path.join(dirName, 'chatterbot.pkl')):
            print('loading from chatterbot.pkl')
            import pickle
            with open(os.path.join(dirName, 'chatterbot.pkl'),'rb') as f:
                self.conversations = pickle.load(f)
        else:
            self.conversations = []
            fileName = os.path.join(dirName, 'greetings.conv')
