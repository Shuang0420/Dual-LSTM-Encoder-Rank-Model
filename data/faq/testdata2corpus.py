...
def clean_text(fname):
    ...
    sh = workbook.sheet_by_name("标准问题，答案")
    nrows = sh.nrows
    row_list = [sh.row_values(i) for i in range(1, nrows)]
    q = ""
    for row in row_list:
        if row[0]:
            q = row[1]
        if row[1] and row[3]:
            userQ.append("%s\t%s\n" % (row[3], row[1]))
        elif row[3] and not row[1]:
            userQ.append("%s\t%s\n" % (row[3], q))
    fw = open("corpus_from_test.txt", "w")
    fw.writelines(userQ)


clean_text("0111校招数据收集-智能回复机器人--测试及结果分析-王程乘20180122.xlsx")
